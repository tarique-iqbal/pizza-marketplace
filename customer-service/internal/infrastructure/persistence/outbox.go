...
func (r *OutboxRepository) WithTx(tx *gorm.DB) outbox.OutboxRepository {
	return &OutboxRepository{db: tx}
}

func (r *OutboxRepository) Create(ctx context.Context, e *outbox.OutboxEvent) error {
	return r.db.WithContext(ctx).Create(e).Error
}

// FetchAndMarkProcessing claims rows matching either: status = 'pending' with next_attempt_at
// elapsed, or status = 'processing' with an expired locked_until lease (never resolved by
// MarkProcessed/ReleaseForRetry/MarkFailed). Both branches increment attempts and set a fresh
// 30s locked_until.
...
func (r *OutboxRepository) ReleaseForRetry(
	ctx context.Context,
	id int64,
	errMsg string,
	delay time.Duration,
