...
Auth, JWT issuance, and user (owner/customer) registration. It is the only service that talks to `postgres-identity` and `redis`. It implements the outbox pattern for its one cross-service-critical event (`restaurant.initiated`) — see root `CLAUDE.md`. `restaurant-service` also implements the pattern now, but for every event it raises, not just one.
...
go run ./cmd/worker   # outbox relay worker — NOT started by compose.yaml, run manually
go test ./...
...
- **Registration is asymmetric between roles** (`internal/application/user/register_owner.go` vs `register_customer.go`): registering an **owner** creates the `User` row and a `restaurant.initiated` outbox row in one DB transaction (see root `CLAUDE.md` for the outbox mechanics), because a restaurant record must reliably get created downstream in `restaurant-service`. Registering a **customer** has no such downstream dependency and just publishes `user.registered` directly/best-effort. Keep this asymmetry in mind if adding new registration flows — the deciding factor is whether another service *must* eventually observe the event.
...
- **Structured logging via `internal/infrastructure/observability/logger`** (`logobs`, ported from restaurant-service — same `NewLogger`/`WithContext`/`FromContext` API): `cmd/api/main.go` uses `gin.New()` + `gin.Recovery()` + `observability.Middleware(logger)` (replacing `gin.Default()`) to inject a request-scoped logger carrying `request_id`/`method`/`path` into each request's context; `cmd/worker/main.go` seeds the same logger into the worker's `ctx` via `WithContext` before starting. `rabbitmq_publisher.go`, `register_owner.go`, and `register_customer.go` all read it back via `FromContext(ctx)` instead of stdlib `log` for reconnect/publish-failure warnings — ctx already flows from the Gin middleware through to these, so no extra wiring was needed to get request-scoped fields on them.
