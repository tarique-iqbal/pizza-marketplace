...
func (cmd *Cancel) Execute(
	...
	if role == "owner" {
		ord, err = cmd.orderRepo.FindByIDAndRestaurantOwner(ctx, orderID, userID)
	} else {
		ord, err = cmd.orderRepo.FindByIDAndCustomer(ctx, orderID, userID)
	}
	if err != nil {
	...
	if ord.PaymentID != nil {
		paymentStatus, err := cmd.paymentProvider.GetPaymentStatus(ctx, *ord.PaymentID)
		if err != nil {
	...
	if ord.PaymentID != nil {
		if err := cmd.paymentProvider.CancelPayment(ctx, *ord.PaymentID); err != nil {
