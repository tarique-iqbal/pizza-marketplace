...
import (
	...
	outboxapp "payment-service/internal/application/outbox"
...
func TestRelay_Process_Success(t *testing.T) {
	...
	e := outbox.OutboxEvent{
		...
		Payload:   []byte(`{"payment_id":"c9b1f7e0-8f7e-4b7c-9b3e-1a2b3c4d5e6f"}`),
	}
	...
	assert.Equal(t, "payment.succeeded", msg.RoutingKey)

	assert.JSONEq(
		t,
		`{"payment_id":"c9b1f7e0-8f7e-4b7c-9b3e-1a2b3c4d5e6f"}`,
		string(msg.Payload),
	)
}
