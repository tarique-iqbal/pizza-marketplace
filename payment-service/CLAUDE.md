...
- **First gRPC surface in this monorepo.** The canonical `.proto` lives in `api/proto/payment/v1/payment.proto`; generated code is committed to `internal/interfaces/grpc/pb/`. No shared Go module for the generated code — order-service's own future gRPC client copies the generated client stub manually, kept in sync by hand (same per-service-duplication convention this repo uses everywhere else, e.g. the RabbitMQ consumer/publisher, the OpenCage client). Transport is plaintext (`insecure.NewCredentials()` equivalent — no TLS setup at all yet), internal network only, no Traefik exposure for the gRPC port.
